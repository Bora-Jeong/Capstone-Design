...
open class Tuple2d : java.io.Serializable, Cloneable {
    /**
     * The x coordinate.
     */
    var x = 0.0F
    ...
    constructor(x: Float, y: Float, confidence: Float) {
        this.x = x
        this.y = y
    ...
    operator fun set(x: Float, y: Float, confidence: Float) {
        this.x = x
        this.y = y
        this.confidence = confidence
    }
     ...
     */
    fun scaleAdd(s: Float, t1: Tuple2d, t2: Tuple2d) {
        x = s * t1.x + t2.x
        y = s * t1.y + t2.y
    }
     ...
     */
    fun scaleAdd(s: Float, t1: Tuple2d) {
        x = s * x + t1.x
        y = s * y + t1.y
    }
     ...
     * @return  true or false
     */
    fun epsilonEquals(t1: Tuple2d, epsilon: Float): Boolean {
     ...
     */
    fun clamp(min: Float, max: Float, t: Tuple2d) {
        x = when {
            t.x > max -> {
                max
            }
     ...
     * @param t   the source tuple, which will not be modified
     */
    fun clampMin(min: Float, t: Tuple2d) {
        x = if (t.x < min) {
            min
        } else {
            t.x
     ...
     * @param t   the source tuple, which will not be modified
     */
    fun clampMax(max: Float, t: Tuple2d) {
        x = if (t.x > max) {
            max
        } else {
            t.x
     ...
     */
    fun interpolate(t1: Tuple2d, t2: Tuple2d, alpha: Float) {
     ...
     * @param alpha  the alpha interpolation parameter
     */
    fun interpolate(t1: Tuple2d, alpha: Float) {
